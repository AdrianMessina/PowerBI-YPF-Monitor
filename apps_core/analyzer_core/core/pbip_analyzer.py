...
class PBIPAnalyzer:
    ...
    def _extract_model_metrics(self, model_data: Dict):
        ...
        print(f"DEBUG: Before storage analysis, len(tables)={len(tables) if tables else 0}")
        if tables:  # solo si hay tablas parseadas
            try:
                from apps_core.analyzer_core.core.storage_mode import analyze_storage
                print(f"DEBUG: Calling analyze_storage with {len(tables)} tables")
                storage = analyze_storage(tables)
                print(f"DEBUG: storage_mode_type = {storage.get('storage_mode_type')}")
                self.metrics['tables_by_mode'] = storage["tables_by_mode"]
                self.metrics['storage_mode_type'] = storage["storage_mode_type"]
                self.metrics['directquery_tables_detail'] = storage["directquery_tables_detail"]
                self.metrics['directquery_issues'] = storage["directquery_issues"]
                self.metrics['query_folding_warnings'] = storage["query_folding_warnings"]
            except Exception as e:
                print(f"WARNING: storage_mode analysis failed — {type(e).__name__}: {e}")
    ...
    def _extract_report_metrics(self, report_data: Dict):
        ...
        # DETECTAR FORMATO: Nuevo (pages/ folder) tiene prioridad sobre Antiguo (sections)
        # Algunos PBIR exportan report.json con sections parciales pero las páginas
        # reales están en carpetas separadas bajo pages/
        definition_path = os.path.join(self.report_path, 'definition')
        if not os.path.exists(definition_path):
            definition_path = self.report_path
        ...
        if os.path.exists(pages_dir) and os.path.exists(pages_json):
            # FORMATO NUEVO (TMDL/PBIR): páginas en carpetas separadas
            self._extract_report_metrics_new_format()
        elif report_data.get('sections', []):
            # FORMATO ANTIGUO (BIM): sections en report.json
            self._extract_report_metrics_old_format(report_data)
        else:
            # Fallback: intentar formato nuevo por si pages/ existe sin pages.json
            self._extract_report_metrics_new_format()
